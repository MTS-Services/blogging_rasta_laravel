<div class="min-h-screen bg-gradient-to-br from-gray-50 via-purple-50 to-pink-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        @if(!config('tiktok.rapidapi_key'))
            <div class="mb-6 bg-gradient-to-r from-red-50 to-orange-50 border-l-4 border-red-500 rounded-lg p-6 shadow-lg">
                <div class="flex items-start">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-red-700 mb-3">
                            RapidAPI key missing. Real TikTok data cannot be loaded.
                        </p>
                        <ol class="list-decimal list-inside space-y-2 text-red-800 mb-4">
                            <li>
                                <a href="https://rapidapi.com/DataFanatic/api/tiktok-scraper7" 
                                   target="_blank" 
                                   class="underline hover:text-red-900 font-semibold">
                                    Subscribe to TikTok Scraper API
                                </a>
                            </li>
                            <li>Copy your API Key</li>
                            <li>Add to .env file: <code class="bg-red-100 px-2 py-1 rounded text-sm">RAPIDAPI_KEY=your_key_here</code></li>
                            <li>Run: <code class="bg-red-100 px-2 py-1 rounded text-sm">php artisan config:clear</code></li>
                        </ol>
                        <a href="https://rapidapi.com/DataFanatic/api/tiktok-scraper7" 
                           target="_blank"
                           class="inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition-colors shadow-md">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-600">
                Latest videos from your featured content creators
            </p>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-8">
            @foreach($featuredUsers as $user)
                @php
                    ...
                    $avatar = $userInfo['avatar_larger'] ?? $userInfo['avatar_thumb'] ?? $userInfo['avatar'] ?? null;
                ...
                >
                    <div class="flex justify-center mb-3">
                        @if($avatar)
                            <img 
                                ...
                                onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name={{ urlencode($nickname) }}&size=200&background={{ $user['color'] }}&color=fff';"
                            >
                        @else
                    ...
                    {{-- Fixed @ symbol rendering using string concatenation --}}
            ...
            <div class="bg-red-50 border-l-4 border-red-400 rounded-lg p-6 mb-8">
                <div class="flex items-start">
                    ...
                    <div class="flex-1">
                        ...
                        @if($this->showDebugInfo() && !empty($debugInfo))
                            <details class="mt-3">
                                <summary class="cursor-pointer text-sm text-red-600 hover:text-red-800">Show debug info</summary>
                                <pre class="mt-2 text-xs bg-red-100 p-3 rounded overflow-x-auto">{{ json_encode($debugInfo, JSON_PRETTY_PRINT) }}</pre>
                            </details>
                        @endif
                    </div>
                </div>
            ...
            @else
                ...
                <div class="bg-white rounded-xl shadow-md border-2 border-dashed border-gray-300 p-16 text-center">
                    ...
                    <p class="text-gray-600 mb-4">
                        @if(!config('tiktok.rapidapi_key'))
                            Please configure your RapidAPI key to load videos
                        @else
                            No videos available for the selected user
                        @endif
                    </p>
                    @if(!config('tiktok.rapidapi_key'))
                        <a href="https://rapidapi.com/DataFanatic/api/tiktok-scraper7" 
