            ...
            @foreach($featuredUsers as $user)
                ...
                >
                    ...
                    <div class="flex justify-center mb-3">
                        @if($profile && isset($profile['user']['avatar']))
                            <img 
                        ...
                        @else
                            <div class="w-16 h-16 rounded-full bg-gradient-to-br from-{{ $user['color'] }}-400 to-{{ $user['color'] }}-600 flex items-center justify-center">
                                <span class="text-white text-xl font-bold">{{ substr($user['username'], 0, 1) }}</span>
                            </div>
                        @endif
                    </div>
                    ...
                    @if($profile && isset($profile['user']['follower_count']))
                        <p class="text-xs text-gray-600 mt-2">
                            {{ $this->formatNumber($profile['user']['follower_count']) }} Followers
                        </p>
                    @endif
                </button>
            @endforeach
        ...
        @else
                            ...
                            <div class="group bg-white rounded-xl shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden transform hover:-translate-y-2">
                                ...
                                <div class="relative aspect-[9/16] bg-gradient-to-br from-pink-100 to-purple-100 overflow-hidden">
                                    ...
                                    @if(isset($video['video']['duration']))
                                        <div class="absolute top-3 right-3 bg-black bg-opacity-75 text-white text-xs font-bold px-2.5 py-1 rounded-md backdrop-blur-sm">
                                            {{ gmdate('i:s', $video['video']['duration']) }}
                                        </div>
                                    @endif

                                    <!-- User Badge -->
                                    <div class="absolute top-3 left-3 bg-white bg-opacity-90 backdrop-blur-sm text-gray-900 text-xs font-semibold px-3 py-1 rounded-full shadow-lg">
                                        {{ $video['_username'] ?? 'unknown' }}
                                    </div>
                                    ...
                                    <div class="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black via-black/70 to-transparent">
                                        <div class="grid grid-cols-3 gap-2 text-white text-xs font-semibold">
                                            @if(isset($video['stats']['play_count']))
                                                <div class="flex items-center justify-center backdrop-blur-sm bg-white/20 rounded-full px-2 py-1">
                                                    ...
                                                    {{ $this->formatNumber($video['stats']['play_count']) }}
                                                </div>
                                            @endif
                                            
                                            @if(isset($video['stats']['digg_count']))
                                                <div class="flex items-center justify-center backdrop-blur-sm bg-white/20 rounded-full px-2 py-1">
                                                    <svg class="w-3.5 h-3.5 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                                        <path fill-rule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clip-rule="evenodd"/>
                                                    </svg>
                                                    {{ $this->formatNumber($video['stats']['digg_count']) }}
                                                </div>
                                            @endif
                                            
                                            @if(isset($video['stats']['comment_count']))
                                                <div class="flex items-center justify-center backdrop-blur-sm bg-white/20 rounded-full px-2 py-1">
                                                    <svg class="w-3.5 h-3.5 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                                        <path fill-rule="evenodd" d="M18 10c0 3.866-3.582 7-8 7a8.841 8.841 0 01-4.083-.98L2 17l1.338-3.123C2.493 12.767 2 11.434 2 10c0-3.866 3.582-7 8-7s8 3.134 8 7zM7 9H5v2h2V9zm8 0h-2v2h2V9zM9 9h2v2H9V9z" clip-rule="evenodd"/>
                                                    </svg>
                                                    {{ $this->formatNumber($video['stats']['comment_count']) }}
                                                </div>
                                            @endif
                                        </div>
                                    </div>
                                ...
                                <div class="p-4">
                                    ...
                                    <div class="flex items-center justify-between text-xs text-gray-500 mb-3">
                                        @if(isset($video['create_time']))
                                            <span class="flex items-center">
                                                ...
                                                {{ \Carbon\Carbon::createFromTimestamp($video['create_time'])->diffForHumans() }}
                                            </span>
                                        @endif
                                    </div>
                                    ...
                                    <!-- View Button -->
                                    <a 
                                        href="https://www.tiktok.com/@{{ $video['_username'] }}/video/{{ $video['video']['id'] ?? $video['id'] }}" 
                                        target="_blank"
            ...
            @else
                ...
                <div class="bg-white rounded-xl shadow-md border-2 border-dashed border-gray-300 p-16 text-center">
                    ...
                    <p class="text-gray-600">This user has no videos or there is an issue accessing the API</p>
                </div>
            @endif
        @endif
    </div>
